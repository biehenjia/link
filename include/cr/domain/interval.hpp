...
namespace cr {
    ...
    struct Field<Interval<T>> {
        ...
        static uint32_t hash(const I& v) {
            return Field<T>::hash(v.lo) ^ (Field<T>::hash(v.hi) * 0x9E3779B1u);
        }

        static bool equal(const I& a, const I& b) {
            return Field<T>::equal(a.lo, b.lo) && Field<T>::equal(a.hi, b.hi);
        }
        ...
        static std::optional<long> whole(const I& v) {
            if (!Field<T>::equal(v.lo, v.hi)) return std::nullopt;
            return Field<T>::whole(v.lo);
        }

        static I add(const I& a, const I& b) {
            return {Field<T>::add(a.lo, b.lo), Field<T>::add(a.hi, b.hi)};
        }

        static I sub(const I& a, const I& b) {
            return {Field<T>::sub(a.lo, b.hi), Field<T>::sub(a.hi, b.lo)};
        }
        ...
        private:
            static bool contains_zero(const I& v) {
                return !(v.hi < Field<T>::zero()) && !(Field<T>::zero() < v.lo);
            }
    };
