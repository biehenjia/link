...
namespace cr {
    ...
    class MpReal {
        public:
            MpReal() { mpfr_init2(v_, precision_); mpfr_set_zero(v_, 1); }
            explicit MpReal(double d) { mpfr_init2(v_, precision_); mpfr_set_d(v_, d, MPFR_RNDN); }
            explicit MpReal(long l) { mpfr_init2(v_, precision_); mpfr_set_si(v_, l, MPFR_RNDN); }

            MpReal(const MpReal& o) { mpfr_init2(v_, precision_); mpfr_set(v_, o.v_, MPFR_RNDN); }
            MpReal(MpReal&& o) noexcept { mpfr_init2(v_, precision_); mpfr_swap(v_, o.v_); }

            MpReal& operator=(const MpReal& o) { mpfr_set(v_, o.v_, MPFR_RNDN); return *this; }
            MpReal& operator=(MpReal&& o) noexcept { mpfr_swap(v_, o.v_); return *this; }
